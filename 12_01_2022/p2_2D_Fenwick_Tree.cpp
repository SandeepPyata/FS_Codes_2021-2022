...
void update(int row,int col,int val,vector<vector<int>>& bit,int n,int m){
    int i=row;
    int j=col;
    while(i<=n){
        while(j<=m){
...
int main(){
    int queries,n,m;cin>>n>>m>>queries;
    ...
    vector<vector<int>> fenwick(n+1,vector<int>(m+1,0));
    
    for(int i=1;i<=n;i++)
        for(int j=1;j<=m;j++)
            update(i,j,vec[i][j],fenwick,n,m);
    
    for(int i=0;i<queries;i++){
        int x1,y1,x2,y2;
        cin>>x1>>y1>>x2>>y2;
        x1++,y1++,x2++,y2++;
        cout<<sum(x1,y1,x2,y2,fenwick)<<endl;
    }
    return 0;
}
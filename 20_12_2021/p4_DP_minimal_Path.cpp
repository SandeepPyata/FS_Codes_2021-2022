...
int dfs(int i,int j,int m, int n, vector<vector<int>> &dp, vector<vector<int>> &vec){
    if(i==m || j==n)
        return INT_MAX;
    ...
    int a = dfs(i+1,j,m,n,dp,vec);
    int b = dfs(i,j+1,m,n,dp,vec);
